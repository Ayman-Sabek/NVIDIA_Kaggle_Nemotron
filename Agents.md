...
- **Memory Fix**: The host only handles 16GB RAM which crashes when buffering 30B param sharding. A massive **64GB NVMe swapfile** was allocated via `/swapfile` to prevent kernel memory panics `g2-standard-4`.
- **State**: Base models successfully bypassed VRAM crash and Sharding loads safely. LoRA finetuning currently ongoing via `tmux` session (`train_fixed_ram`) on the Kaggle mirrored model.
